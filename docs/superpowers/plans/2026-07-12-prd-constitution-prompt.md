# `/prd-constitution-prompt` Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Adicionar ao harness-prd a skill `/prd-constitution-prompt` — ponte para o Passo 5a (bootstrap) que monta o prompt do `/speckit.constitution` derivando princípios decidíveis dos NFRs/ADRs da PRD, entrega e para.

**Architecture:** Espelha `/prd-specify-prompt` (mesma anatomia de 4 fases, delega a `rewrite-prompt`, referencia âncoras da fonte única `.specify/prd/quality-rules.md`, para no handoff). A guarda muda de "sem-stack" para "decidível-não-genérico ∧ rastreável a NFR/ADR". Adiciona duas âncoras à fonte única e sincroniza os dois guias.
...
|---|---|---|
| `.claude/skills/prd-constitution-prompt/SKILL.md` | Criar | A skill-ponte do Passo 5a (4 fases + handoff). |
...
| `docs/guia-prd-para-spec-kit.md` | Modificar | Nota no Passo 5a; linha `rewrite-prompt` cita P5a+P5b. |
...
### Task 2: Criar a skill `prd-constitution-prompt`

**Files:**
- Create: `.claude/skills/prd-constitution-prompt/SKILL.md`

- [ ] **Step 1: Criar o diretório e o arquivo com o conteúdo completo**

Crie `.claude/skills/prd-constitution-prompt/SKILL.md` com exatamente:
...
Run: `test -f .claude/skills/prd-constitution-prompt/SKILL.md && grep -E 'name: prd-constitution-prompt|rewrite-prompt|#anatomia-constitution|PARE AQUI' .claude/skills/prd-constitution-prompt/SKILL.md`
Expected: as 4 linhas presentes (`name:`, `rewrite-prompt`, a referência `#anatomia-constitution`, e `PARE AQUI`).

- [ ] **Step 3: Commit**

```bash
git add .claude/skills/prd-constitution-prompt/SKILL.md
git commit -m "feat(prd): comando prd-constitution-prompt (ponte para o Passo 5a)"
```
...
```markdown
| `/prd-constitution-prompt` | Ponte p/ 5a (bootstrap, 1×) | `docs/PRD.md` (NFRs+ADRs) | prompt do `/speckit.constitution` | `rewrite-prompt` |
| `/prd-specify-prompt` | Ponte p/ 5b | backlog de fatias | prompt do `/speckit.specify` | `rewrite-prompt` |
```
...
Localize `### Ponte — /prd-specify-prompt` e insira **antes** dela:

````markdown
### Ponte (bootstrap, 1×) — `/prd-constitution-prompt`

Roda **uma vez por projeto**, depois que a PRD tem NFRs e restrições de ADRs. Deriva princípios
**decidíveis** deles:

```text
/prd-constitution-prompt Enfatize render e persistência; derive o resto dos NFRs.
```

Delega a `rewrite-prompt` montando as tags de `#anatomia-constitution` e **entrega o comando pronto**
(não dispara nada):
...
`/prd-constitution-prompt` entrega o `/speckit.constitution` (bootstrap, 1×) e
`/prd-specify-prompt` entrega o `/speckit.specify` (por fatia). O ciclo do Spec Kit é seu.
```
...
```markdown
5. `/prd-constitution-prompt` (1×) → `/speckit.constitution "..."` pronto → **você** dispara o bootstrap.
6. `/prd-specify-prompt <fatia>` → `/speckit.specify "..."` pronto → **você** dispara o Spec Kit.
```

- [ ] **Step 6: Verificar as edições**

Run: `grep -cE 'prd-constitution-prompt' docs/como-usar-o-harness-prd.md`
...
git commit -m "docs(harness): guia de uso reflete a ponte /prd-constitution-prompt"
```
...
- [ ] **Step 2: Atualizar a linha `rewrite-prompt` na tabela de skills**
...
Expected: 2 linhas — a nota da ponte no Passo 5a e a menção "(P5a)" na linha do `rewrite-prompt`.

- [ ] **Step 4: Commit**

```bash
git add docs/guia-prd-para-spec-kit.md
git commit -m "docs(guia): Passo 5a cita a ponte /prd-constitution-prompt"
```
...
Run: `test -f .claude/skills/prd-constitution-prompt/SKILL.md && grep -q 'constitution-prompt' .specify/prd/quality-rules.md && grep -q 'anatomia-constitution' .specify/prd/quality-rules.md && echo OK`
Expected: `OK`

- [ ] **Step 2: Confirmar que a skill referencia a âncora que existe na fonte única**

Run: `grep -q '#anatomia-constitution' .claude/skills/prd-constitution-prompt/SKILL.md && grep -q '{#anatomia-constitution}' .specify/prd/quality-rules.md && echo REF-OK`
...
**Cobertura do spec:**
- Skill `prd-constitution-prompt` com 4 fases + handoff → Task 2. ✓
...
- Delegar a `rewrite-prompt` → Fase 2/3 da skill (Task 2). ✓
...
- `guia`: nota no Passo 5a + linha `rewrite-prompt` cita P5a → Task 4. ✓
