...
**Tech Stack:** Markdown com frontmatter YAML (convenção de skill do Claude Code, igual a `.claude/skills/adr-new/SKILL.md`). Nada compila; a verificação é observacional (ler o artefato + exercitar cada comando num cenário real).
...
- `.claude/skills/prd-discovery/SKILL.md` — Estágio 1. (Task 3)
- `.claude/skills/prd-spike/SKILL.md` — Estágio 2. (Task 4)
- `.claude/skills/prd-write/SKILL.md` — Estágio 3. (Task 5)
- `.claude/skills/prd-decompose/SKILL.md` — Estágio 4. (Task 6)
- `.claude/skills/prd-specify-prompt/SKILL.md` — ponte para 5b. (Task 7)
...
Ordem: fundações compartilhadas (Task 1–2) → comandos (Task 3–7) → migração do guia (Task 8) → verificação ponta-a-ponta (Task 9). Cada comando depende de `quality-rules.md`; `prd-write` depende de `prd-skeleton.md`; `prd-decompose` depende de `traceability-table.md`.
...
### Task 3: `/prd-discovery` — Estágio 1

**Files:**
- Create: `.claude/skills/prd-discovery/SKILL.md`
...
Run: `test -f .claude/skills/prd-discovery/SKILL.md && echo EXISTE || echo AUSENTE`
Expected: `AUSENTE`
...
Run: `grep -E 'name: prd-discovery|superpowers:brainstorming|#criterios-de-conclusao' .claude/skills/prd-discovery/SKILL.md`
Expected: 3 linhas (frontmatter, delegação, referência às regras).

- [ ] **Step 5: Commit**

```bash
git add .claude/skills/prd-discovery/SKILL.md
git commit -m "feat(prd): comando prd-discovery (estágio 1)"
```

---

### Task 4: `/prd-spike` — Estágio 2

**Files:**
- Create: `.claude/skills/prd-spike/SKILL.md`

- [ ] **Step 1: Critério de aceitação**

Skill que: (0) exige `docs/discovery.md`; (1) filtra 2–3 decisões estruturantes; (2/3) auto-delega a
`deep-research` e depois a `adr-new` por decisão; (4) confere que cada decisão virou um ADR que
referencia um spike real.

- [ ] **Step 2: Verifique que ainda não existe**

Run: `test -f .claude/skills/prd-spike/SKILL.md && echo EXISTE || echo AUSENTE`
Expected: `AUSENTE`
...
Run: `grep -E 'name: prd-spike|deep-research|adr-new' .claude/skills/prd-spike/SKILL.md`
Expected: 3 linhas.

- [ ] **Step 5: Commit**

```bash
git add .claude/skills/prd-spike/SKILL.md
git commit -m "feat(prd): comando prd-spike (estágio 2)"
```

---

### Task 5: `/prd-write` — Estágio 3 (o coração)

**Files:**
- Create: `.claude/skills/prd-write/SKILL.md`
...
Run: `test -f .claude/skills/prd-write/SKILL.md && echo EXISTE || echo AUSENTE`
Expected: `AUSENTE`
...
Run: `grep -E 'name: prd-write|prd-skeleton.md|#fronteira|modo revisar' .claude/skills/prd-write/SKILL.md`
Expected: 4 linhas (frontmatter, cópia do template, guarda de fronteira, idempotência).

- [ ] **Step 5: Commit**

```bash
git add .claude/skills/prd-write/SKILL.md
git commit -m "feat(prd): comando prd-write (estágio 3, guarda de fronteira)"
```

---

### Task 6: `/prd-decompose` — Estágio 4

**Files:**
- Create: `.claude/skills/prd-decompose/SKILL.md`
...
Run: `test -f .claude/skills/prd-decompose/SKILL.md && echo EXISTE || echo AUSENTE`
Expected: `AUSENTE`
...
Run: `grep -E 'name: prd-decompose|#invest|traceability-table.md|walking skeleton' .claude/skills/prd-decompose/SKILL.md`
Expected: 4 linhas.

- [ ] **Step 5: Commit**

```bash
git add .claude/skills/prd-decompose/SKILL.md
git commit -m "feat(prd): comando prd-decompose (estágio 4)"
```

---

### Task 7: `/prd-specify-prompt` — Ponte para 5b

**Files:**
- Create: `.claude/skills/prd-specify-prompt/SKILL.md`

- [ ] **Step 1: Critério de aceitação**

Skill que: (0) exige backlog de fatias e recebe qual fatia; (1) valida que a fatia tem resultado
observável e avisa se cita stack; (2/3) auto-delega a `rewrite-prompt` montando o XML com as 3 tags;
(4) entrega o `/speckit.specify "..."` pronto como **handoff** e NÃO dispara `/speckit.*`.

- [ ] **Step 2: Verifique que ainda não existe**

Run: `test -f .claude/skills/prd-specify-prompt/SKILL.md && echo EXISTE || echo AUSENTE`
Expected: `AUSENTE`
...
Run: `grep -E 'name: prd-specify-prompt|rewrite-prompt|#anatomia-specify|PARE AQUI' .claude/skills/prd-specify-prompt/SKILL.md`
Expected: 4 linhas (incluindo a barreira de handoff).

- [ ] **Step 5: Commit**

```bash
git add .claude/skills/prd-specify-prompt/SKILL.md
git commit -m "feat(prd): comando prd-specify-prompt (ponte para o Spec Kit)"
```
...
- [ ] **Cenário 1 — Caminho feliz encadeado.** Rode `/prd-discovery` com a semente do Zion (de
  `docs/index.md`) → confirme que `docs/discovery.md` nasce com "não faz" explícito → `/prd-spike` →
  confirme ADR em `docs/adr/` → `/prd-write` → confirme que copiou o esqueleto e preencheu RF-xx →
  `/prd-decompose` → confirme fatias + tabela injetada na seção 12 → `/prd-specify-prompt <fatia>` →
  confirme o `/speckit.specify` pronto. **Evidência:** cada artefato existe e cada Fase 4 deu `✓`.

- [ ] **Cenário 2 — Gate mole dispara e não trava.** Sem `docs/discovery.md`, rode `/prd-write`.
  **Evidência:** a Fase 0 avisa "recomendo `/prd-discovery` antes" e **pergunta se segue**; ao
...
- [ ] **Cenário 3 — Detecção de fronteira vazada.** Force um RF com "usar React para..." em
  `docs/PRD.md` e rode `/prd-write` em modo revisar. **Evidência:** a Fase 4 aponta a linha exata e
  sugere movê-la para o `plan.md`, citando `#fronteira`.

- [ ] **Cenário 4 — Idempotência / revisar.** Com `docs/PRD.md` já existente, rode `/prd-write`.
  **Evidência:** entra em modo *pressionar seção*, não sobrescreve o arquivo do zero.

- [ ] **Cenário 5 — INVEST reprova fatia horizontal.** Dê ao `/prd-decompose` uma fatia "só a UI".
  **Evidência:** aponta que falha no teste "dá demo sozinha?" e sugere SPIDR.

- [ ] **Cenário 6 — Handoff termina o território.** Rode `/prd-specify-prompt`. **Evidência:**
...
`#anatomia-specify`) e os nomes de skill (`prd-discovery`, `prd-spike`, `prd-write`, `prd-decompose`,
`prd-specify-prompt`) batem entre as tasks que os definem (1–2) e as que os referenciam (3–9).
```
